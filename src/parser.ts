  ...
  private parseProperties(): ParamsAst {
    ...
    while (this.check("IDENT") && this.peekAhead()?.type === "EQUALS") {
      ...
      if (!this.check("IDENT") && !this.check("STRING")) {
        throw new SyntaxError("Expected property value (IDENT or STRING)", this.peek());
      }
      const value = this.value(this.advance());
      params[key] = value;
    }
    return params;
  }
...
export function parse(input: string): Ast {
  ...
  for (const directive of directives) { // TODO: assert keys, map to map
    ...
    if (typeSeen)
      throw new SyntaxError("Duplicate '.type' directive", zeroLocation);
