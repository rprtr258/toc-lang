...
const tocLangParserPromise = loadFile(tocLangGrammarUrl).then(peggyGrammar => peggy.generate(peggyGrammar));
...
const isGoalNodeStatement = (s) => s.type === "node" && normalizeId(s.id) === "goal";
...
export function parseProblemTreeSemantics(ast): TreeSemantics {
  const nodes = new Map<string, Node>();
  const edges = [] as Edge[];
...
export async function parseTextToAst(code: string): Promise<ParseResult> {
